...
public:
    ...
    // MStar Android Patch Begin
    void  forceDrawCallFinish() const;
    void  setGLViewPort(int xoffset, int yoffset, int width, int height) const;
    uint32_t initCaptureData(const sp<IMemory>& captureData, int opTimingWidth, int opTimingHeight);
    void releaseCaptureData(uint32_t textrue);
    // MStar Android Patch End
};
