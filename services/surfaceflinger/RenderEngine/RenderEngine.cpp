...
void RenderEngine::forceDrawCallFinish() const {
    return ;
}

void RenderEngine::setGLViewPort(int xoffset, int yoffset, int width, int height) const {
    return ;
}
#else
void RenderEngine::forceDrawCallFinish() const {
...
uint32_t RenderEngine::initCaptureData(const sp<IMemory>& captureData, int opTimingWidth, int opTimingHeight) {
    uint32_t cameraCaptureTexture;
    if (captureData != NULL) {
        ssize_t offset;
        size_t size;
        sp<IMemoryHeap> heap = captureData->getMemory(&offset, &size);
        /*uint32_t * address = (uint32_t *)malloc(opTimingWidth*opTimingHeight*4);
        memcpy(address, (uint8_t *)heap->base() + offset, opTimingWidth*opTimingHeight*4);
        for (int i = 0;i < opTimingWidth*opTimingHeight;i++) {
            *address= 0xFF0000FF;
            address++;
        }*/
        glGenTextures(1, &cameraCaptureTexture);
        glEnable(GL_TEXTURE_2D);
        //glDisable(GL_TEXTURE_EXTERNAL_OES);
        glBindTexture(GL_TEXTURE_2D, cameraCaptureTexture);
        if ((heap!=NULL)&&(heap->base()!=MAP_FAILED)) {
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, opTimingWidth, opTimingHeight, 0, GL_RGBA,
                     GL_UNSIGNED_BYTE,(uint8_t *)heap->base() + offset);
        } else {
           ALOGE("error!!!,allocate the IMemoryHEAP for capturedata failed");
        }
    ...
    return cameraCaptureTexture;
}
void RenderEngine::releaseCaptureData(uint32_t textrue){
    glDeleteTextures(1, &textrue);
}
