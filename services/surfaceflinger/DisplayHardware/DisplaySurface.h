...
public:
    ...
    // MStar Android Patch Begin
    virtual int getOsdWidth() const = 0;
    virtual int getOsdHeight() const = 0;
#ifdef BUILD_FOR_STB
    virtual void reSetPanelSize(int width, int height) = 0;
    virtual status_t reSetGopSize(int width, int height, int hstart, int interleave, int orientation, int value) = 0;
    virtual status_t getAdjustValue(int *adjustwidth, int *adjustheight, int* adjustloffset, int* adjusttopoffset) = 0;
#endif
    virtual void recalc3DTransform(Transform** first, Transform** second, Rect& frame) =0;
    virtual void getCurTiming(int *width, int *height) =0;
    // MStar Android Patch End
