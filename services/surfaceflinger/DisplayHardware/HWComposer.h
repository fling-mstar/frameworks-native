...
public:
    ...
    // MStar Android Patch Begin
    int setGopStretchWin(int32_t gopNo, int32_t dest_Width, int32_t dest_Height);
    int setDisplayMode(int displayMode);
    int getCurOPTiming(int *width, int *height);
    int getUrsaVsersion();
    int isStbTarget();
    int isOsdNeedResize();
#ifdef BUILD_MSTARTV_MI
    int CurOsdWidth,CurOsdHeight;
    int getCurOSDRegion(int *width, int *height);
#endif
#ifdef BUILD_FOR_STB
    void reSetPanelSize(int panelwidth, int panelheight) const;
    status_t reSetGopSize(int width, int height, int hstart, int interleave, int orientation, int value);
    status_t getAdjustValue(int *adjustwidth, int *adjustheight,int* adjustloffset,int* adjusttopoffset);
    status_t  setReproduceRate(int value);
#endif
    // MStar Android Patch End
    ...
    public:
        ...
        // MStar Android Patch Begin
        virtual uint32_t getVendorUsageBits() = 0;
        // MStar Android Patch End
    };
    ...
    // MStar Android Patch Begin
    int getOsdWidth(int disp) const;
    int getOsdHeight(int disp) const;
    // MStar Android Patch End
...
private:
    ...
    // MStar Android Patch Begin
#ifdef ENABLE_HWCURSOR
    hwcursor_device_t* mHwCursorDev;
    int mHwCursorVaddr;
    int mHwCursorWidth;
    int mHwCursorHeight;
    int mHwCursorStride;
...
public:
    ...
    int hwCursorShow();
    int hwCursorHide();
    int hwCursorSetMatrix(float dsdx, float dtdx, float dsdy, float dtdy);
    int hwCursorSetPosition(float positionX, float positionY, float hotSpotX, float hotSpotY, int iconWidth, int iconHeight);
    int hwCursorSetAlpha(float alpha);
    int hwCursorChangeIcon();
    int hwCursorDoTransaction();
    int hwCursorGetWidth() {return mHwCursorWidth;}
    int hwCursorGetHeight() {return mHwCursorHeight;}
    int hwCursorGetStride() {return mHwCursorStride;}
    void* hwCursorGetIconVaddr() {return (void*)mHwCursorVaddr;}
    int hwCursorChangeResolution();
    int hwCursorsetOverscan(int left, int top, int right,int bottom);
    int loadHwCursorModule();
#endif
    int require4k2kOutputTiming(int enable);
    status_t updateFramebufferTargetFrameSize(int32_t id);
    status_t updateFramebufferTargetFrameSize(int32_t id, Rect &rect);
    bool mSwitchTimingEnable;
    void recalc3DTransform(Transform** first, Transform** second, Rect& frame);
    // MStar Android Patch End
};
