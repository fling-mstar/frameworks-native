...
void FrameTracker::setActualPresentTime(nsecs_t presentTime) {
    ...
    nsecs_t timeDiff = mFrameRecords[mOffset].actualPresentTime
        - mFrameRecords[(mOffset+NUM_FRAME_RECORDS-1) % NUM_FRAME_RECORDS].actualPresentTime;
    mFrameRecords[mOffset].fps = (timeDiff == 0) ? -1 : static_cast<int>(s2ns(1)/timeDiff);
