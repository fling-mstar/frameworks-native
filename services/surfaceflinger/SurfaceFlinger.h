...
public:
    ...
    // MStar Android Patch Begin
    friend class ScreenCaptureListener;
    // MStar Android Patch End
...
private:
    ...
    // MStar Android Patch Begin
    class VSyncMessageMonitor: public Thread {
    protected:
        sp<SurfaceFlinger> mFlinger;
        sp<IDisplayEventConnection> mVSyncEvent;
        sp<BitTube> mmVSyncEventTube;
        sp<Looper> mLooper;
    public:
        VSyncMessageMonitor();
        void Init(sp<SurfaceFlinger> surFlinger,
    ...
    virtual status_t  forcePostSecFb(int secFbId);
    virtual status_t  getSecFbBuffer(sp<GraphicBuffer>* buf, int idx);
    virtual status_t setGopStretchWin(int32_t gopNo, int32_t dest_Width, int32_t dest_Height);
    void wrapDrawLayer(const sp<Layer>& layer, const sp<const DisplayDevice>& hw, const Region& dirty, int compositionType, int displayMode);
    ...
    void renderScreenImplLocked(
            ...
            uint32_t reqWidth, uint32_t reqHeight,
            uint32_t minLayerZ, uint32_t maxLayerZ,
            bool yswap, uint32_t supportWidth = 1920,
            uint32_t supportHeight = 1080);
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    bool doDisplayComposition(const sp<const DisplayDevice>& hw, const Region& dirtyRegion);
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    mutable Mutex mFrameAvailableLock;
    sp<IMemory> mCaptureData;
    Condition mFrameAvailableCV;
    // MStar Android Patch End
    State mCurrentState;
    ...
    // MStar Android Patch Begin
    VSyncMessageMonitor mVsyncMonitor;
    // MStar Android Patch End
    ...
    bool mDaltonize;
    // MStar Android Patch Begin
    mutable Mutex mOverscanLock;
    mutable int mLeftOverscan, mTopOverscan, mRightOverscan, mBottomOverscan;
public:
    HWComposer& getHwComposer() { return *mHwc; }
#ifdef ENABLE_HWCURSOR
    ...
    virtual status_t changeHwCursorResolution();
    virtual status_t loadHwCursorModule();
private:
    sp<MemoryBase> mHwcursorIcon;
#endif
private:
    bool mChangeTo2k1kMode;
    bool m4k2kAnd2k1kCoexistEnable;
#ifdef ENABLE_STR
    bool mStrBootAnimExit;
    bool mStandbyRecording;
#endif
public:
    bool isChangeTo2k1kMode(){ return mChangeTo2k1kMode;}
    bool get4k2kAnd2k1kSwitch(){ return m4k2kAnd2k1kCoexistEnable;}
    void getScaleParamByCurrentResolutionMode(float* hScale, float* vScale) const;
    sp<DisplayDevice> getBuiltInDisplayDevice(int32_t id) {
        return getDisplayDevice(getBuiltInDisplay(id));
    }
    // MStar Android Patch End
};
