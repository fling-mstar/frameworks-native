                            ...
                            EGLint *num_config)
{
    ...
    *num_config = 0;
    ...
    property_get("mstar.debug.egl.force_depth", value, "false");
    if (!strcmp(value, "true")) {
        EGLint numConfigRGBA32;
        ...
        EGLBoolean resRGBA32 = cnx->egl.eglChooseConfig(
            dp->disp.dpy, attribs, configs, config_size, &numConfigRGBA32);
        if (resRGBA32 == EGL_TRUE && numConfigRGBA32 > 0) {
            ALOGD("force set depth & stencil buffer ");
            *num_config = numConfigRGBA32;
            return resRGBA32;
        }
    }
          ...
          EGLConfig config, const EGLint *attrib_list)
{
    ...
    egl_display_ptr dp = validate_display_connection(dpy, cnx);
    // MStar Android Patch Begin
    if (dp && cnx->egl.eglCreatePbufferFromClientBuffer) {
        EGLSurface surface = cnx->egl.eglCreatePbufferFromClientBuffer(
